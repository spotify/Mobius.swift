...
public final class MobiusController<Model, Event, Effect> {
    ...
    init(
        ...
        self.loopQueue = DispatchQueue(label: "MobiusController \(Model.self)", target: loopQueue)
    ...
    public func connectView<C: Connectable>(_ connectable: C) where C.InputType == Model, C.OutputType == Event {
        synchronized {
            ...
            viewConnectable = AsyncDispatchQueueConnectable(connectable, acceptQueue: viewQueue, replyQueue: loopQueue)
        }
    }
